...
from .modeling_beacon import Memory, cat_tensor
import random
import copy
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # return keys and values before rope; lzy: and after catting
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def _beacon_forward(self, 
        ...
        # local loss
        input_ids, attention_mask, past_key_values, labels = self.memory.retrieve_local_ans()
    ...
    def inference_forward(self, 
        ...
        # add_mask = True #correspnd to beacon size=2
        if self.memory.input_ids is not None: #input_ids contains only one token
            self.memory.beacon_activations = [(None, None) for _ in range(self.memory.num_layers)]
            for chunk_idx in range(self.memory.chunk_number):
                input_ids_w_pre = torch.cat([self.memory.last_input_ids,input_ids],dim=-1)
                # if add_mask:
                #     input_ids_w_pre = torch.cat([input_ids_w_pre, torch.tensor([2]).unsqueeze(0).cuda()],dim=-1)
                attention_mask = torch.ones_like(input_ids_w_pre)
                past_key_values = []
                raw_kvs = self.memory.raw_activations[chunk_idx]
                for i in range(self.memory.num_layers):
                    key = raw_kvs[i][0].cuda()
                    value = raw_kvs[i][1].cuda()
                    layer_past_key_values = (key, value, self.memory.beacon_size, self.memory.local_window, self.memory.local_window)
                    past_key_values.append(layer_past_key_values)
                memory_size = past_key_values[0][0].shape[self.memory.k_seq_dim]
                attention_mask = torch.cat([attention_mask.new_ones(attention_mask.shape[0], memory_size), attention_mask], dim=1)
                outputs = self._native_forward(
                    input_ids=input_ids_w_pre,
                    attention_mask=attention_mask,
                    position_ids=position_ids,
                    past_key_values=past_key_values,
                    inputs_embeds=inputs_embeds,
                ...
                self.memory.update_memory(outputs.past_key_values, chunk_idx,first_time_inferenece=False)
            self.memory.last_input_ids = input_ids
            local_a_input_ids, local_a_attention_mask, past_key_values, _ = self.memory.retrieve_local_ans()
            if self.memory.generated_id_cache is not None:
                input_ids = torch.cat([self.memory.generated_id_cache,input_ids],dim=-1)
            local_a_input_ids = torch.cat([local_a_input_ids,input_ids],dim=-1)
            local_a_attention_mask = torch.cat([local_a_attention_mask,torch.ones_like(input_ids)],dim=-1)
            
            outputs = self._native_forward(
                input_ids=local_a_input_ids,
                attention_mask=local_a_attention_mask,
                position_ids=position_ids,
                past_key_values=past_key_values,
                inputs_embeds=inputs_embeds,
                use_cache=use_cache,
            ...
            outputs = self.memory.output(outputs)
            self.memory.generated_id_cache = input_ids
            return outputs
        ...
        if self.memory.sequence_length < 4096:
            self.memory.local_query_start = 0
            self.memory.local_size = self.memory.sequence_length
            self.memory.chunk_number = 0
        else:
            assert len(self.memory.memory_stride) == 1
            stride = self.memory.memory_stride[0]
            self.memory.local_size = self.memory.local_window
            self.memory.local_query_start = self.memory.sequence_length - self.memory.local_size
            self.memory.sequence_length = self.memory.sequence_length - self.memory.local_size

            self.memory.chunk_number = (self.memory.sequence_length - self.memory.sequence_length % stride) // stride
            if self.memory.sequence_length % stride!=0:
                self.memory.chunk_number += 1
            #prepare raw_activations
            self.memory.raw_activations = [[(None, None) for _ in range(self.memory.num_layers)] for i in range(self.memory.chunk_number)]
        ...
        for chunk_idx in range(self.memory.chunk_number):
            ...
            # update past_key_values
            self.memory.update_memory(outputs.past_key_values,chunk_idx)
        ...
        self.memory.last_input_ids = local_a_input_ids[...,-1:]
        outputs = self.memory.output(outputs)
        return outputs

    def inference_ppl_forward(self, 
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        ...
        reserve_size = 128
        input_ids_for_pred = copy.deepcopy(input_ids[:, -reserve_size:])
        input_ids = input_ids[:, :-reserve_size]
        labels = input_ids.clone()
        self.memory.prepare(
            input_ids=input_ids, 
            attention_mask=attention_mask, 
        ...
        local_a_input_ids, local_a_attention_mask, past_key_values, labels = self.memory.retrieve_local_ans()
        labels = torch.cat([labels,input_ids_for_pred[...,:1]],dim=-1)
        outputs = self._native_forward(
            input_ids=local_a_input_ids,
            attention_mask=local_a_attention_mask,
            position_ids=position_ids,
        ...
        if labels is not None:
            self.memory.update_loss(outputs.batch_loss, outputs.valid_token_num)
        self.memory.last_input_ids = local_a_input_ids[...,-1:]
        for pred_i in range(input_ids_for_pred.shape[-1]-1):#the last token cannot compute loss
            torch.cuda.empty_cache()
            input_ids = input_ids_for_pred[...,pred_i:pred_i+1] #only one token
            self.memory.beacon_activations = [(None, None) for _ in range(self.memory.num_layers)]
            for chunk_idx in range(self.memory.chunk_number):
                input_ids_w_pre = torch.cat([self.memory.last_input_ids,input_ids],dim=-1)
                attention_mask = torch.ones_like(input_ids_w_pre)
            ...
            local_a_input_ids = torch.cat([local_a_input_ids,input_ids],dim=-1)
            local_a_attention_mask = torch.cat([local_a_attention_mask,torch.ones_like(input_ids)],dim=-1)
            local_labels = torch.full(local_a_input_ids.shape, -100)
            local_labels[..., -1:] = input_ids_for_pred[..., pred_i + 1:pred_i + 2]
            outputs = self._native_forward(
                input_ids=local_a_input_ids,
                attention_mask=local_a_attention_mask,
                position_ids=position_ids,
    ...
    def forward(self, **kwargs):
        ...
        if self.training:
            with optional_grad_ctx(with_grad=self.training):
                rdn = random.choice([1,2])
                if rdn == 1: #loss 1
                    return self._beacon_forward(**kwargs,autoencoder=False)
                elif rdn == 2: #loss 2
                    return self._beacon_forward(**kwargs,autoencoder=True)
        else:
            with torch.no_grad():
